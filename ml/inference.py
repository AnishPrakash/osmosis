...
class AnomalyScorer:
    ...
    def _load(self):
        ...
        if iso_path.exists() and scaler_path.exists():
            with open(iso_path,    "rb") as f: self.model  = pickle.load(f)
            ...
            print("[OSmosis] ⚠ No model found — run 'make train' after ~10 min baseline. Scoring disabled.")
    ...
    def score(self, process_stats: dict) -> float:
        ...
        if raw >= t_med:  return 0.1                                   # Normal
        if raw >= t_high: return 0.65                                  # Suspicious
