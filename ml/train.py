...
def rebuild_process_stats() -> list:
    ...
    stats = defaultdict(lambda: {
        ...
        "sched_preemptions": 0, "write_count": 0, "open_count": 0,
        "fork_count": 0, "mmap_count": 0, "page_allocs": 0, "page_frees": 0,
        "vfs_writes": 0, "vfs_renames": 0, "syscall_types": set(),
    })
    ...
    for (payload,) in cursor:
        try:
            ...
            if etype == "syscall":
                ...
                if me == "page_fault":
                    ps["page_faults"] += 1
                    if evt.get("is_major"): ps["major_faults"] += 1
...
def train():
    ...
    if X.shape[0] < 10:
        print("[OSmosis] ⚠ Too few processes for reliable training. Collect more baseline data.")
        return
    ...
    print(f"[OSmosis] Calibrated thresholds → Med: {threshold_med:.4f}, High: {threshold_high:.4f}")

    # Save artifacts
    with open(MODEL_DIR / "iso_forest.pkl", "wb") as f: pickle.dump(iso_forest, f)
    ...
    for i, name in enumerate(["log_syscall", "log_page_faults", "log_major_faults",
                               "log_sched", "write_ratio", "open_ratio", "fork_ratio",
                               "mmap_ratio", "diversity", "rename_ratio",
                               "leak_pressure", "major_fault_ratio", "vfs_intensity"]):
        print(f"  [{i:2d}] {name}")
